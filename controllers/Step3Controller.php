...
class Step3Controller {
    ...
    public function actionIndex() {
        ...
        $playersCardsComb = $this->combineCardsForPlayers($playersCards);
        $playersRangArray = $this->analiseALL($playersCardsComb);
        $playersRang = $this->printCombinations($playersRangArray);

        require_once ROOT . '/views/viewStep3.php';
        return TRUE;
    }
    ...
    private function combineCardsForPlayers($playersCards) {

        $combine = function ($playerCards) {

            $flop = $_SESSION['flop'];
            $j = count($playerCards) + 1;
            $limit = count($playerCards) + count($flop);

            while ($j <= $limit) {
                $playerCards[$j] = $flop[$j - 3];
                $j++;
            }
            ...
            array_multisort($rangCart, SORT_DESC, $playerCards);

            return $playerCards;
        };

        $playersCards = array_map($combine, $playersCards);
